...
function mouseClick(press) {
  if (press.target.matches("[data-key]")) {
    pressKey(press.target.dataset.key)
    return
  }

  if (press.target.matches("[data-enter]")) {
    enterGuessWord()
    return
  }

  if (press.target.matches("[data-delete]")) {
    deleteKey()
    return
  }
}
...
function pressKey(key) {
  ...
  if (currentTiles.length >= 5) return
  const nextTile = gameBox.querySelector(":not([data-letter])")
...
function deleteKey() {
  const currentTiles = getCurrentTiles()
  const lastLetter = currentTiles[currentTiles.length - 1]
...
function enterGuessWord() {
  ...
  const guess = currentTiles.reduce((word, tile) => {
  ...
  stopGame()
  currentTiles.forEach((tile, index, array) => flipTile(tile, index, array, guess))
}
